...
import haxe.ui.toolkit.controls.Button2.Button2Layout;
...
class Button2 extends StateComponent implements IFocusable implements IClonable<StateComponent> {
	/**
	...
	private static var _groups:StringMap<Array<Button2>>;
	...
	public function new() {
		...
		if (_groups == null) {
			_groups = new StringMap<Array<Button2>>();
		}
	}
	...
	private function get_iconPosition():String {
		if (Std.is(_layout, Button2Layout)) {
			return cast(_layout, Button2Layout).iconPosition;
		}
		return "";
	}
	...
	private function set_iconPosition(value:String):String {
		if (Std.is(_layout, Button2Layout)) {
			cast(_layout, Button2Layout).iconPosition = value;
		}
		return value;
	}
	...
	private function set_selected(value:Bool):Bool {
		if (_toggle == true && _selected != value) {
			...
			if (_group != null && value == true) {
				var arr:Array<Button2> = _groups.get(_group);
				if (arr != null) {
					for (button in arr) {
						if (button != this) {
	...
	private function set_group(value:String):String {
		if (value != null) {
			var arr:Array<Button2> = _groups.get(value);
			if (arr != null) {
				arr.remove(this);
			}
		}
	...
	public override function applyStyle():Void {
		...
		if (_style != null) {
			...
			if (_style.iconPosition != null && Std.is(_layout, Button2Layout)) {
				cast(_layout, Button2Layout).iconPosition = _style.iconPosition;
				invalidate(InvalidationFlag.LAYOUT);
			}

			if (_style.labelPosition != null && Std.is(_layout, Button2Layout)) {
				cast(_layout, Button2Layout).labelPosition = _style.labelPosition;
			}
		}
	}
	...
	// Helpers
	//******************************************************************************************
	private static function optionInGroup(value:String, option:Button2):Bool {
	...
	private function resizeButton(force:Bool = false):Void {
		if (text.length > 0 && autoSize == true && Std.is(_layout, Button2Layout)) {
			var buttonLayout:Button2Layout = cast(_layout, Button2Layout);
...
@exclude
class Button2Layout extends Layout {
