...
class DefaultStyles extends Styles {
	public function new() {
		...
		addStyle("Button:over, AccordionButton:over, List:over, Date:over, CalendarDay:over", new Style( { backgroundColor: 0xAAAAAA } ));
		addStyle("Button:down, AccordionButton:down, List:down, Date:down, CalendarDay:down", new Style( { backgroundColor: 0x888888 } ));
		addStyle("VScroll", new Style( { width: 17, height: 100, hasButtons: false} ));
		addStyle("HScroll", new Style( { width: 100, height: 17, hasButtons: false } ));
		addStyle("ListView #even", new Style( { backgroundColor: 0xEEEEEE } ));
